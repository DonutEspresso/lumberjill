...
function SimpleStream(options) {
     ...
     * @type {Boolean}
     */
    self.forcestdout = options.forcestdout;
}
...
SimpleStream.prototype.write = function(record) {
    const self = this;
    const log = this.forcestdout === true ? console.log : console.warn;

    // log in raw JSON format
    if (self.raw === true) {
        log(stringify(record, true));
    } else {
        // write to stderr
...
function create(options) {
    ...
    assert.optionalBool(options.forcestdout, 'options.forcestdout');
